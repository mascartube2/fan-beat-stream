...
BEGIN
  ...
  -- Prochaine piste avec pochette, après la dernière publiée
  SELECT t.* INTO _track
  FROM public.tracks t
  WHERE t.cover_path IS NOT NULL
    AND (_row.last_track_id IS NULL
         OR t.created_at > (SELECT created_at FROM public.tracks WHERE id = _row.last_track_id))
  ORDER BY t.created_at ASC
  LIMIT 1;

  -- Rotation : recommence depuis la plus ancienne avec pochette
  IF _track.id IS NULL THEN
    SELECT * INTO _track FROM public.tracks WHERE cover_path IS NOT NULL ORDER BY created_at ASC LIMIT 1;
  END IF;

  -- Fallback total : prend n'importe quelle piste
  IF _track.id IS NULL THEN
